              ...
              <div :class="['field-widget', element.focus ? 'field-widget-focus' : '']" @click="onWidgetClick(element)">
                <template v-if="element.focus">
                  <div class="widget-move">
                    <icon-drag-arrow :size="12" />
                  </div>
                  <div class="widget-delete" @click.stop.prevent="onWidgetDelete(element)">
                    <icon-close :size="12" />
                  </div>
                </template>
        ...
        <a-form :model="widget" layout="vertical">
          ...
          <a-form-item field="options" label="选项" v-if="[WIDGET.SINGLE_CHOICE, WIDGET.MULTI_CHOICE].includes(widget.type)">
            <div class="select-options-box">
              <div class="option-item" v-for="(option, idx) in widget.options">
                <div class="option">
                  <a-input v-model:model-value="widget.options[idx]" :max-length="16" />
                </div>
                <div class="delete" v-if="widget.options.length > 1" @click="onDeleteWidgetOption(widget, idx)">
                  <icon-close :size="16" />
                </div>
              </div>
          ...
          <a-form-item label="格式" v-if="[WIDGET.MONEY].includes(widget.type)">
            <a-grid :cols="1">
              ...
              <a-grid-item>
                <a-checkbox v-model:model-value="widget.comma">显示千位分隔符</a-checkbox>
              </a-grid-item>
            </a-grid>
          </a-form-item>
