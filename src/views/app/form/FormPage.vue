    ...
    <a-card>
      <a-row>
        <a-col :flex="1">
          <a-form :model="queryParam" :label-col-props="{ span: 8 }" :wrapper-col-props="{ span: 16 }" label-align="right">
            <a-row>
              <a-col :span="6">
                <a-form-item field="name" label="页面名称">
                  <a-input v-model="queryParam.name" placeholder="请输入页面名称" allow-clear />
                </a-form-item>
              </a-col>
            </a-row>
          </a-form>
      ...
      >
        <template #columns>
          ...
          <a-table-column title="页面名称" data-index="name" align="center" width="200"></a-table-column>
          <a-table-column title="页面地址" data-index="url" align="center" ></a-table-column>
...
import { queryFormPage, deleteFormPage } from "@/api/app/form";
import FormPageAdd from "./FormPageAdd.vue";
import FormPageEdit from "./FormPageEdit.vue";
...
const loadTableData = async () => {
  try {
    loading.value = true;
    const { data, page } = await queryFormPage(tablePagination, queryParam);
...
const handleDelete = async (record)=> {
  try {
    loading.value = true;
    const { success, message } = await deleteFormPage(record.id);
    if (success) {
