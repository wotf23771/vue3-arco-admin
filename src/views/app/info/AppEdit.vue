...
import { getAppById,updateApp } from "@/api/app/app";
...
const initdata = async (record) => {
  ...
  const { success, data, message } = await getAppById(id.value);
  if (success) {
     form.appName = data.appName;
    form.appId = data.appId;
    form.appSecret = data.appSecret;
    form.pcHost = data.pcHost;
    form.mobileHost = data.mobileHost;
    form.procCallback = data.procCallback;
    form.taskCallback = data.taskCallback;
...
const handleSubmit = async ()=>{
  ...
  try {
    ...
    loading.value = true;
    const { code, success, message } = await updateApp(id.value,form);
    if (!success) {
