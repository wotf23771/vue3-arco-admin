      ...
      <a-row :gutter="16">
        ...
        <a-col span="11">
          <a-form-item field="isLocked" label="锁定状态" label-col-flex="80px" :rules="[{ required: true, message: '锁定状态为空' }]">
            <a-select v-model="form.isLocked" placeholder="请选择" allow-clear>
        ...
        <a-col span="11">
          <a-form-item field="isEnabled" label="启用状态" label-col-flex="80px" :rules="[{ required: true, message: '启用状态为空' }]">
            <a-select v-model="form.isEnabled" placeholder="请选择" allow-clear>
...
import { saveMainOrgUser } from "../../api/orgUserApi";
...
const handleSubmit = async () => {
  try {
    ...
    loading.value = true;
    const { success, message } = await saveMainOrgUser(orgId.value, form);
    if (!success) {
